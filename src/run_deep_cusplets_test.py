...
def train_test_dow30():
    ...
    fit_model = deep_cusplets.create_and_train_mlp_model(Xtrain, ytrain, savename='traindow_testdow_mlp_fd700')
    ...
    for i in range(len(ts)):
        ...
        axes[i].plot(cusp_indic[feature_dim:], color='k', linewidth=2)
        axes[i].plot(preds, linewidth=0.5, color='r')

    plt.savefig( f'./deep_cusplets_figures/traindow_testdow.png' )


def train_test_labmt(load=False):
    ...
    rand_inds = np.random.choice( range(ranks.shape[0]), size=200, replace=False )
    ...
    if load is False:
        ...
        fit_model = deep_cusplets.create_and_train_mlp_model(Xtrain, ytrain, savename='trainlabmt_testlabmt_mlp_fd750')

    else:
        fit_model = joblib.load( './models/trainlabmt_testlabmt_mlp_fd750.joblib.lzma' )
    ...
    for i, (ax, ind) in enumerate( zip(axes, test_inds) ):
        ...
        testrank = ranks[ind]        
        preds = deep_cusplets.predict( fit_model, testrank, feature_dim )
    ...
    plt.tight_layout()
    plt.savefig('./deep_cusplets_figures/trainlabmt_testlabmt.pdf')
