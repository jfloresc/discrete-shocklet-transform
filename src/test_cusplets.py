...
from . import cusplets


def test_cusplet_width_input():
    arr = np.random.randn(100)
    kernel = cusplets.power_zero_cusp
...
def test_classify_cusps():
    ...
    extrema, sum_cc, gez = cusplets.classify_cusps(cc, b=0.75, geval=0.5)
    assert len(gez) > 0
    assert max(sum_cc) > 1
...
def test_make_components():
    ...
    extrema, sum_cc, gez = cusplets.classify_cusps(cc, b=0.75, geval=0.5)
    windows, estimated_cusp_pts = cusplets.make_components(gez,
            extrema)
